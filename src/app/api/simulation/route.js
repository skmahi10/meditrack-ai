...
// Generate a reroute path — offset from the main route midpoints
function generateRerouteCoords(origin, destination, route) {
  // Find a city that's off the main corridor to route through
  ...
  for (const [name, coords] of Object.entries(CITY_DATABASE)) {
    if (name === origin.city || name === destination.city) continue;
    // Find existing route cities
    const isOnRoute = route.some((r) => r.city === name);
    if (isOnRoute) continue;
    
    const dStart = Math.sqrt(Math.pow(coords.lat - origin.lat, 2) + Math.pow(coords.lng - origin.lng, 2));
    const dEnd = Math.sqrt(Math.pow(coords.lat - destination.lat, 2) + Math.pow(coords.lng - destination.lng, 2));
    const dTotal = Math.sqrt(Math.pow(destination.lat - origin.lat, 2) + Math.pow(destination.lng - origin.lng, 2));
    
    // City should be reachable but off the direct path
    if (dStart < dTotal * 1.2 && dEnd < dTotal * 1.2 && dStart > dTotal * 0.2) {
      const score = dStart + dEnd; // prefer cities not too far off
      if (score < bestScore || bestScore === -Infinity) {
        bestScore = score;
        detourCity = { name, lat: coords.lat, lng: coords.lng };
      }
    }
  }

  if (!detourCity) {
    // Fallback: create an offset midpoint
    const offsetLat = midLat + (Math.random() > 0.5 ? 1.5 : -1.5);
    const offsetLng = midLng + (Math.random() > 0.5 ? 1.5 : -1.5);
    detourCity = { name: "Detour", lat: offsetLat, lng: offsetLng };
  }

  // Reroute: from ~35% point through detour city back to ~65% point
  const rerouteStart = positionAt(32, route);
  const rerouteEnd = positionAt(58, route);

  return {
    via: detourCity.name,
    coords: [
      [rerouteStart.lat, rerouteStart.lng],
      [detourCity.lat, detourCity.lng],
      [rerouteEnd.lat, rerouteEnd.lng],
    ],
  };
}
...
const TELEMETRY_TIMELINE = [
  ...
  { sec: 17, temp: -17.7, hum: 43, spd: 22,  bat: 88, prog: 32 },  // ← traffic detected here
  { sec: 19, temp: -17.2, hum: 45, spd: 15,  bat: 86, prog: 35 },  // ← rerouting, slow
  { sec: 21, temp: -17.0, hum: 44, spd: 40,  bat: 85, prog: 38 },  // reroute active
  { sec: 23, temp: -15.0, hum: 48, spd: 55,  bat: 45, prog: 42 },  // temp breach starts
  { sec: 25, temp: -12.0, hum: 51, spd: 40,  bat: 25, prog: 45 },
  { sec: 27, temp: -5.0,  hum: 55, spd: 15,  bat: 15, prog: 48 },  // worst breach
  { sec: 29, temp: -8.0,  hum: 53, spd: 30,  bat: 22, prog: 52 },
  { sec: 31, temp: -14.0, hum: 49, spd: 58,  bat: 45, prog: 56 },
  { sec: 33, temp: -17.0, hum: 46, spd: 72,  bat: 45, prog: 63 },  // recovered
  { sec: 35, temp: -17.4, hum: 44, spd: 85,  bat: 46, prog: 70 },
  { sec: 37, temp: -17.8, hum: 43, spd: 90,  bat: 50, prog: 75 },
  { sec: 39, temp: -17.6, hum: 42, spd: 75,  bat: 53, prog: 82 },
  { sec: 41, temp: -17.3, hum: 43, spd: 48,  bat: 56, prog: 88 },
  { sec: 43, temp: -17.9, hum: 44, spd: 62,  bat: 60, prog: 94 },
  { sec: 45, temp: -17.5, hum: 43, spd: 35,  bat: 63, prog: 98 },
  { sec: 47, temp: -17.2, hum: 42, spd: 0,   bat: 65, prog: 100 },
];
...
export async function POST(request) {
  try {
    ...
    // Generate reroute data
    const rerouteData = generateRerouteCoords(
      originalShipment.origin || { city: "Mumbai", lat: 19.076, lng: 72.877 },
      originalShipment.destination || { city: "Delhi", lat: 28.614, lng: 77.209 },
      ROUTE
    );
...
sendShipmentCreated("mahishaik567@gmail.com", originalShipment).catch(() => {});

    const aiPromise = (async () => {
      const incidentContext = { ...originalShipment, status: "at-risk", currentTemp: -5, riskScore: 72 };
    ...
    await writeBlock("SHIPMENT_CREATED", { shipmentId, product: originalShipment.product, origin: originalShipment.origin, destination: originalShipment.destination });

    await paymentRef.set({
      id: paymentRef.id, paymentId, shipmentId,
      ...
      amount: 125000, currency: "INR", status: "held",
      conditions: { deliveryConfirmed: false, tempCompliance: true, chainVerified: false, signatureReceived: false },
      disputeReason: null, disputeSeverity: null, blockchainRef: null,
      createdAt: originalShipment.createdAt || Timestamp.now(),
      heldAt: Timestamp.now(), releasedAt: null,
    }, { merge: true });
    ...
    // ==========================================
    // PHASE 2: IN TRANSIT — telemetry + traffic + breach
    // ==========================================
    for (const t of TELEMETRY_TIMELINE) {
      ...
      // ═══════════════════════════════════════════
      // TRAFFIC DETECTED at ~32% (sec 17)
      // ═══════════════════════════════════════════
      if (t.prog === 32 && !trafficDetected) {
        trafficDetected = true;
        updateData.riskScore = 38;
        ...
        await writeBlock("TRAFFIC_DETECTED", {
          shipmentId,
          location: positionAt(32, ROUTE),
          severity: "heavy",
          estimatedDelay: "25 minutes",
        });

        await writeNotification("alert", "warning", "Traffic congestion detected",
          `Heavy traffic detected on ${shipmentId} route. AI analyzing alternate paths.`,
          originalShipment.carrierId
        );
      }

      // ═══════════════════════════════════════════
      // REROUTE ACTIVE at ~35% (sec 19)
      // ═══════════════════════════════════════════
      if (t.prog === 35 && !rerouteActive) {
        rerouteActive = true;
        updateData.rerouted = true;
        updateData.rerouteCoords = JSON.stringify(rerouteData.coords);
        updateData.rerouteVia = rerouteData.via;
        updateData.riskScore = 30;
        updateData.riskFactors = {
          temperature: 10,
          delay: 35,
          route: 30,
        ...
        await writeBlock("REROUTE_ACTIVATED", {
        shipmentId,
        via: rerouteData.via,
        reason: "traffic_congestion",
        newRoute: JSON.stringify(rerouteData.coords),
      });

        await writeNotification("system", "info", "Route optimized",
          `${shipmentId} rerouted via ${rerouteData.via} to avoid traffic. ETA adjusted.`,
          originalShipment.carrierId
        );
      }
      ...
      if (t.temp === -5.0) {
        ...
        updateData.riskFactors = { temperature: 85, delay: 20, route: 10, weather: 15, cooling: 90, transit: 30 };
        ...
        await writeBlock("TEMP_VIOLATION", { shipmentId, temperature: -5, threshold: originalShipment.tempRange, duration: "8 seconds" });
        await paymentRef.update({
          status: "on-hold",
          conditions: { deliveryConfirmed: false, tempCompliance: false, chainVerified: false, signatureReceived: false },
        });

        await writeNotification("alert", "critical", "Temperature breach detected", `${shipmentId} reached -5\u00B0C, outside ${originalShipment.tempRange?.min}\u00B0C to ${originalShipment.tempRange?.max}\u00B0C range.`, originalShipment.carrierId);
        await writeNotification("alert", "critical", "Temperature breach detected", `${shipmentId} reached -5\u00B0C and requires quality inspection.`, originalShipment.receiverId);

        sendBreachAlert("mahishaik567@gmail.com", originalShipment, { temperature: -5, batteryLevel: 15 }).catch(() => {});
      }

      // AI recommendation after breach
      if (t.temp === -8.0) {
        const { aiRecommendation } = await aiPromise;
        await writeBlock("AI_RECOMMENDATION", { shipmentId, recommendation: aiRecommendation, riskReduction: "72 to 45" });
      }
      ...
      if (t.sec === 33) {
        ...
        updateData.riskFactors = { temperature: 45, delay: 20, route: 10, weather: 10, cooling: 35, transit: 25 };
      ...
      if (t.prog === 100) {
        ...
        updateData.rerouted = false;
        updateData.rerouteCoords = null;

        await writeBlock("DELIVERED", { shipmentId, receivedBy: originalShipment.receiverId, temperature: -17.2, condition: "accepted" });
    ...
    try {
      complianceReport = await generateComplianceReport({ ...originalShipment, status: "delivered" }, [], []);
    } catch {
      complianceReport = `Compliance report: Shipment ${shipmentId} delivered from ${originCity} to ${destCity} with one temperature breach incident and one traffic reroute. Overall cold-chain integrity maintained.`;
    }
    ...
    await paymentRef.update({
      status: "released",
      conditions: { deliveryConfirmed: true, tempCompliance: true, chainVerified: true, signatureReceived: true },
      releasedAt: Timestamp.now(),
    });

    await writeBlock("PAYMENT_RELEASED", { paymentId, shipmentId, amount: 125000, conditions: { deliveryConfirmed: true, tempCompliance: true, chainVerified: true, signatureReceived: true } });
    await writeNotification("payment", "info", "Payment released", `${paymentId} released after delivery, signature, and chain verification.`, originalShipment.createdBy);

    sendDeliveryConfirmation("mahishaik567@gmail.com", { ...originalShipment, status: "delivered", complianceReport }).catch(() => {});

    return Response.json({
      success: true,
      message: `Simulation complete for ${shipmentId} (${originCity} to ${destCity})`,
      totalTelemetryPoints: TELEMETRY_TIMELINE.length,
      totalBlocks: nextBlockNumber - 1,
      durationSeconds: Math.round((Date.now() - start) / 1000),
      route: ROUTE.map((r) => r.city).join(" \u2192 "),
      rerouteVia: rerouteData.via,
    });
  } catch (error) {
